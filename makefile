...
EXCLUDE_FROM_SOURCE="bin,.xcodeproj,obj,src\/addons\/ofx3DUtils\/example"
...
LDFLAGS = $(USER_LD_FLAGS) $(LIB_PATHS_FLAGS) -Wl,-rpath=./libs
...
ifeq ($(findstring addons.make,$(wildcard *.make)),addons.make)
	...
	ADDONS_INCLUDES += $(shell find $(OTHER_ADDONS_PATH)/*/src/ -type d)
	ADDONS_INCLUDES += $(shell find $(OTHER_ADDONS_PATH)/*/libs/ -type d)
	...
	ADDONS_LIBS_STATICS += $(shell ls $(OTHER_ADDONS_PATH)/*/libs/*/lib/$(LIBSPATH)/*.a)
	ADDONS_LIBS_SHARED += $(shell ls $(OTHER_ADDONS_PATH)/*/libs/*/lib/$(LIBSPATH)/*.so)
	...
	OTHER_ADDONS_DIRS = $(addprefix $(OTHER_ADDONS_PATH)/, $(ADDONS_REL_DIRS) )
	OTHER_ADDONS_LIBS_DIRS = $(addprefix $(OTHER_ADDONS_PATH)/, $(ADDONS_LIBS_REL_DIRS) )
	...
	ADDONS_SOURCES += $(shell find $(OTHER_ADDONS_DIRS) -name "*.cpp" -or -name "*.c")
	ADDONS_SOURCES += $(shell find $(OTHER_ADDONS_LIBS_DIRS) -name "*.cpp" -or -name "*.c" 2>/dev/null)
